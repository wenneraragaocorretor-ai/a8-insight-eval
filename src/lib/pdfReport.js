...
const BG_SOFT = [247, 248, 250]; // #F7F8FA
const CARD_TOP = [255, 255, 255]; // white card
const CARD_BOTTOM = [247, 248, 250];
const CARD_BLUE = [230, 241, 251]; // #E6F1FB light blue
const BORDER = [232, 232, 232]; // #E8E8E8
const GOLD = [200, 169, 81]; // #C8A951
const BLUE = [15, 45, 92]; // #0F2D5C
const NAVY = [10, 31, 68]; // #0A1F44
const GOLD_LIGHT = [226, 201, 126]; // #E2C97E
const WHITE = [255, 255, 255];
const TEXT = [44, 44, 42]; // #2C2C2A
const GRAY = [90, 95, 105];
const GRAY_DIM = [140, 145, 155];
...
const fmtBRL = (v) => v == null || isNaN(Number(v))
    ? "—"
    : Number(v).toLocaleString("pt-BR", { style: "currency", currency: "BRL", maximumFractionDigits: 0 });
function calcularRegressao(avaliacao, comparaveis, resultado) {
    ...
    const pts = (comparaveis || [])
        ...
        .filter((p) => p !== null);
    const n = pts.length;
    const base = { ok: false, n, a: 0, b: 0, r2: 0, valorM2: 0, valorTotal: 0, areaAvaliada };
    if (n < 2) {
...
function aplicarRegressao(resultado, avaliacao, comparaveis) {
...
const fmtNum = (v, digits = 2) => v == null || isNaN(Number(v)) ? "—" : Number(v).toLocaleString("pt-BR", { maximumFractionDigits: digits });
...
export function modelosDisponiveis(plano) {
    const p = String(plano || "basico").toLowerCase();
    if (p === "expert")
        return [1, 2, 3];
    if (p === "profissional" || p === "pro")
        return [1, 2];
    return [1];
}
export function podeGerarModelo(plano, modelo) {
    return modelosDisponiveis(plano).includes(modelo);
}
// ---------- helpers ----------
function pintarFundo(doc) {
    doc.setFillColor(...BG);
...
function card(doc, x, y, w, h, opts = {}) {
    ...
    const fill = variant === "darkblue" ? BLUE : variant === "blue" ? CARD_BLUE : WHITE;
...
function tituloPagina(doc, texto, y = 28, cor = BLUE) {
...
function microHeader(doc, corretor, laudoNum) {
...
function marcaDagua(doc) {
    ...
    for (let i = 1; i <= total; i++) {
        ...
        doc.setFontSize(80);
        doc.setTextColor(...GOLD);
        doc.text("A8 AVALIA", PW / 2, PH / 2, { align: "center", angle: 30 });
        if (gs && doc.setGState) {
...
function rodape(doc) {
    ...
    for (let i = 2; i <= total; i++) {
        ...
        doc.text("Esta avaliação é mercadológica e não substitui laudo técnico assinado por profissional habilitado (CNAI/IBAPE).", PW / 2, PH - 7, { align: "center" });
...
function textoMultilinha(doc, texto, x, y, w, opts = {}) {
    const size = opts.size ?? 10;
    doc.setFont("helvetica", opts.bold ? "bold" : "normal");
    doc.setFontSize(size);
    doc.setTextColor(...(opts.color ?? GRAY));
    const lh = opts.lineHeight ?? size * 0.42;
    let lines = doc.splitTextToSize(texto, w);
...
// ============================================================
function iconCircle(doc, cx, cy, r, glyph, cor = GOLD) {
...
function progressBar(doc, x, y, w, pct, cor = GOLD) {
...
function gaugeChart(doc, cx, cy, r, valor) {
    const v = Math.max(0, Math.min(10, valor));
    const segs = 40;
    const start = Math.PI;
    const end = 0;
    const colorFor = (t) => t < 0.4 ? [220, 53, 69] : t < 0.7 ? [240, 180, 60] : [40, 167, 105];
    doc.setLineWidth(5);
    for (let i = 0; i < segs; i++) {
        ...
        const filled = t <= v / 10;
        const col = filled ? colorFor(t) : [232, 232, 232];
...
function trendBadge(doc, cx, cy, w, h, tendencia) {
    const cor = tendencia === "alta" ? [40, 167, 105]
...
function infraIcon(doc, kind, cx, cy, r, ok) {
    const cor = ok ? [40, 167, 105] : [200, 200, 205];
    doc.setFillColor(...cor);
    doc.circle(cx, cy, r, "F");
...
function checkIcon(doc, cx, cy, r, ok) {
    const cor = ok ? [40, 167, 105] : [200, 200, 205];
    doc.setFillColor(...cor);
    doc.circle(cx, cy, r, "F");
    if (ok) {
...
function badgePadrao(padrao) {
    const p = String(padrao || "").toLowerCase();
    if (p.includes("luxo"))
        return { cor: [120, 81, 169], label: padrao };
    if (p.includes("alto"))
        return { cor: GOLD, label: padrao };
    if (p.includes("normal") || p.includes("médio") || p.includes("medio"))
        return { cor: BLUE, label: padrao };
    return { cor: [120, 125, 135], label: padrao || "—" };
}
function conservPct(c) {
    const v = String(c || "").toLowerCase();
    if (v.includes("péssimo") || v.includes("pessimo") || v.includes("ruim"))
        return 0.18;
...
// ---------- PAGE 1: COVER ----------
function paginaCapa(doc, avaliacao, corretor, titulo, capaFoto) {
    pintarFundo(doc);
    ...
    doc.text("Conforme orientações da NBR 14653-2 da ABNT", PW / 2, yTituloCentro + 16, { align: "center" });
...
// ---------- PAGE: SUMÁRIO ----------
function paginaSumario(doc, sec) {
    novaPagina(doc);
    doc.setFont("helvetica", "bold");
    doc.setFontSize(54);
    doc.setTextColor(...BLUE);
...
// ---------- PAGE: IMÓVEL ----------
function paginaImovel(doc, a, rel, corretor) {
    novaPagina(doc);
    ...
    if (hasTotal)
        areaItems.push(["ÁREA TOTAL (m²)", String(a.area_total)]);
    if (hasConstr)
        areaItems.push(["ÁREA CONSTR. (m²)", String(a.area_construida)]);
    if (hasPriv)
        areaItems.push(["ÁREA PRIV. (m²)", String(a.area_privativa)]);
    if (areaItems.length === 0)
        areaItems.push(["ÁREA (m²)", "—"]);
    const items = [
    ...
    // pros / cons
    const pos = Array.isArray(rel?.pontos_positivos) ? rel.pontos_positivos : [];
    const neg = Array.isArray(rel?.pontos_atencao)
        ? rel.pontos_atencao
...
function paginaFotos(doc, rel, fotos, corretor) {
    novaPagina(doc);
    microHeader(doc, corretor);
    ...
    const drawImg = (src, x, y, w, h) => {
        try {
            ...
            doc.addImage(src, fmt, x + 1, y + 1, w - 2, h - 2, undefined, "FAST");
        }
        catch (e) {
    ...
    const analiseTxt = typeof rel?.analise_fotos === "string" && rel.analise_fotos.trim().length > 0
        ? rel.analise_fotos
...
function paginaBairro(doc, a, rel, corretor) {
    novaPagina(doc);
    microHeader(doc, corretor);
    ...
    const infraAtivos = Array.isArray(ab.infraestrutura)
        ? ab.infraestrutura.map((s) => String(s).toLowerCase())
    ...
    const tend = tendStr.includes("desv") || tendStr.includes("baix")
        ? "baixa"
        : tendStr.includes("estav") ? "estavel" : "alta";
    trendBadge(doc, x3 + colW / 2, yRow + 36, colW - 16, 20, tend);
    textoMultilinha(doc, String(ab.tendencias_mercado ?? rel?.tendencias_mercado ?? "Mercado em movimento positivo, com demanda crescente na região."), x3 + 6, yRow + 56, colW - 12, { size: 9, color: TEXT, lineHeight: 4.2, maxHeight: ch - 60 });
...
function paginaPerfil(doc, rel, corretor) {
    novaPagina(doc);
    microHeader(doc, corretor);
    ...
    const stripNonAscii = (s) => s.replace(/[\uD800-\uDBFF][\uDC00-\uDFFF]/g, "") // surrogate pairs (emojis)
    ...
    const interesses = Array.isArray(pp.interesses)
        ? pp.interesses.filter(Boolean).map(String)
        : String(pp.interesses ?? pp.preferencias ?? "").split(/[|,]+/).map((s) => s.trim()).filter(Boolean);
    if (interesses.length) {
        ...
        let cxi = M;
        let cyi = 175;
        const cores = [BLUE, GOLD, [120, 81, 169], [40, 167, 105], [220, 100, 80]];
...
function paginaAnuncios(doc, comparaveis, corretor, tipoImovel) {
    ...
    for (let p = 0; p < Math.ceil(comparaveis.length / PER_PAGE); p++) {
        ...
        pageItems.forEach((c, idx) => {
            ...
            if (ref > 0 && vm > 0) {
                ...
                const cor = t < 0.4 ? [40, 167, 105] : t > 0.6 ? [220, 53, 69] : GOLD;
                doc.setFillColor(...cor);
...
function paginaValor(doc, resultado, corretor) {
    novaPagina(doc);
    // Fundo navy total
    doc.setFillColor(...NAVY);
    doc.rect(0, 0, PW, PH, "F");
...
// ---------- PAGE: CONTATO ----------
function paginaContato(doc, corretor) {
    novaPagina(doc);
    ...
    const detectarLabel = (v) => {
        const s = v.trim();
        if (/^cau\b/i.test(s))
            return { label: "CAU", value: stripPrefix(s, ["CAU"]) };
        if (/^crea\b/i.test(s))
            return { label: "CREA", value: stripPrefix(s, ["CREA"]) };
        return { label: "Registro", value: s };
    };
    const registros = [];
    if (corretor.creci)
        registros.push(`CRECI ${stripPrefix(corretor.creci, ["CRECI"])}`);
    if (corretor.cnai)
        registros.push(`CNAI ${stripPrefix(corretor.cnai, ["CNAI"])}`);
    ...
    doc.text("Esta avaliação é mercadológica e não substitui laudo técnico", PW / 2, PH - 22, { align: "center" });
    doc.text("aprovado por profissional habilitado (CNAI/IBAPE).", PW / 2, PH - 17, { align: "center" });
    // Faixa dourada inferior
    doc.setFillColor(...GOLD);
    doc.rect(0, PH - 4, PW, 4, "F");
}
// ---------- EXPERT EXTRA: HOMOGENEIZAÇÃO ----------
function paginaHomogeneizacao(doc, a, comparaveis, corretor) {
    novaPagina(doc);
    microHeader(doc, corretor);
    ...
    const rank = (lista, v) => { const i = lista.indexOf(v); return i === -1 ? 0 : i; };
    const rows = comparaveis.map((c, i) => {
        const fOferta = 0.9;
    ...
    rows.forEach((r, ri) => {
        ...
        r.fatores.forEach((f, fi) => {
            ...
            const cor = val < 1 ? BLUE : val > 1 ? GOLD : [120, 125, 135];
            const delta = val - 1;
            const half = barAreaW / 2;
            const len = Math.min(half, Math.abs(delta) * half / 0.2);
            doc.setFillColor(...cor);
            if (delta < 0)
                doc.rect(refX - len, by, len, barH, "F");
            else if (delta > 0)
                doc.rect(refX, by, len, barH, "F");
            else
                doc.rect(refX - 0.4, by, 0.8, barH, "F");
            ...
            doc.text(fmtNum(val, 2), valX, by + barH / 2 + 1.4, { align: align });
        });
        if (ri < rows.length - 1) {
            doc.setDrawColor(...BORDER);
            doc.setLineWidth(0.2);
...
function paginaEstatistica(doc, comparaveis, corretor, tipoImovel, resultado) {
    novaPagina(doc);
    microHeader(doc, corretor);
    ...
    const cvColor = cv < 15 ? [40, 167, 105] : cv < 30 ? [240, 180, 60] : [220, 53, 69];
    const usable = PW - M * 2;
    const gap = 6;
    ...
    cards.forEach((c, i) => {
        ...
        doc.setFont("helvetica", "bold");
        doc.setFontSize(9);
        doc.setTextColor(...(isLight ? GRAY : [220, 220, 220]));
        ...
        doc.setFont("helvetica", "normal");
        doc.setFontSize(8);
        doc.setTextColor(...(isLight ? GRAY : [200, 200, 200]));
    ...
    cursorY = textoMultilinha(doc, `Tratamento por fatores de homogeneização (ABNT NBR 14653-2). Coef. de variação de ${fmtNum(cv, 1)}% — ${cvDesc}.`, M, cursorY, usable, { size: 11, color: TEXT, lineHeight: 5.2 }) ?? cursorY + 10;
    ...
    if (reg && reg.ok) {
        ...
        const r2Color = r2 > 0.8 ? [80, 200, 140] : r2 >= 0.6 ? [240, 200, 80] : [240, 120, 120];
        doc.setFont("helvetica", "bold");
        doc.setFontSize(9);
        ...
        textoMultilinha(doc, `Equação: ${reg.equacao || ""} · R²: ${reg.r2.toFixed(4)} (${reg.interpretacao || ""}). n = ${reg.n} comparáveis.`, M, boxY + boxH + 6, usable, { size: 9, color: GRAY, lineHeight: 4.6 });
    }
    else if (resultado) {
        textoMultilinha(doc, "Regressão linear não calculada: amostra insuficiente (mínimo 2 comparáveis com área e valor válidos).", M, cursorY + 8, usable, { size: 10, color: GRAY, lineHeight: 5 });
    }
}
// ---------- EXPERT EXTRA: CAMPO DE ARBÍTRIO ----------
function paginaArbitrio(doc, resultado, corretor) {
    novaPagina(doc);
    microHeader(doc, corretor);
    ...
    textoMultilinha(doc, "Conforme item 9.2.3 da ABNT NBR 14653-2, é admitido um campo de arbítrio de ±15% sobre o valor central como margem técnica de ajuste, sem alterar o grau de fundamentação da avaliação.", M, yRow + ch + 18, usable, { size: 11, color: TEXT, lineHeight: 5.4 });
}
// ============================================================
// Orquestração dos modelos
// ============================================================
function gerarModelo1(avaliacao, resultado, comparaveis, corretor, fotos, fotosDet = []) {
    ...
    if (temDocFotos)
        paginaDocumentacaoFotografica(doc, fotosDetLim, corretor);
    else if (temFotos)
        paginaFotos(doc, rel, fotosLim, corretor);
...
function gerarModelo2(avaliacao, resultado, comparaveis, corretor, fotos, fotosDet = []) {
    const doc = new jsPDF({ unit: "mm", format: "a4", orientation: "portrait" });
    const rel = resultado?.relatorio_json || {};
...
function paginaFichaTecnica(doc, a, corretor) {
    novaPagina(doc);
    microHeader(doc, corretor);
    ...
    if (vagas > 0)
        principais.push({ icon: "", value: String(vagas), label: "Vagas" });
    if (a.area_total)
        principais.push({ icon: "", value: String(a.area_total), label: "m² Área" });
    ...
    // ===== ACABAMENTOS =====
    const acab = Array.isArray(a.tipo_acabamento)
        ? a.tipo_acabamento.filter((s) => typeof s === "string" && s.trim().length > 0)
        : [];
    if (acab.length) {
        doc.setFont("helvetica", "bold");
        doc.setFontSize(10);
    ...
    // ===== INFRA LAZER =====
    const lazer = Array.isArray(a.infraestrutura_lazer)
        ? a.infraestrutura_lazer.filter((s) => typeof s === "string" && s.trim().length > 0)
        : [];
    if (lazer.length) {
        doc.setFont("helvetica", "bold");
        doc.setFontSize(10);
...
function paginaAmbientes(doc, a, corretor) {
    const sociais = Array.isArray(a.ambientes_sociais)
        ? a.ambientes_sociais.filter((s) => typeof s === "string" && s.trim().length > 0)
        : [];
    const servico = Array.isArray(a.ambientes_servico)
        ? a.ambientes_servico.filter((s) => typeof s === "string" && s.trim().length > 0)
        : [];
    const outros = Array.isArray(a.ambientes_outros)
        ? a.ambientes_outros.filter((s) => typeof s === "string" && s.trim().length > 0)
        : [];
    if (sociais.length === 0 && servico.length === 0 && outros.length === 0)
        return;
    novaPagina(doc);
    microHeader(doc, corretor);
...
function paginaDocumentacaoFotografica(doc, fotosDet, corretor) {
    if (fotosDet.length === 0)
        return;
    const usable = PW - M * 2;
    const gap = 6;
    const colW = (usable - gap) / 2;
    ...
    fotosDet.forEach((f) => {
        ...
        try {
            ...
            doc.addImage(f.dataUrl, fmt, x + 1, y + 1, colW - 2, imgH - 2, undefined, "FAST");
        }
        catch (e) {
            console.error("Falha ao desenhar foto:", e);
        }
...
function paginaDispersao(doc, avaliacao, resultado, comparaveis, corretor) {
    novaPagina(doc);
    microHeader(doc, corretor);
    ...
    const avalY = Number(resultado?.valor_unitario_medio) ||
    ...
    const sx = (x) => x0 + ((x - xMin) / (xMax - xMin || 1)) * cw;
    const sy = (y) => y1 - ((y - yMin) / (yMax - yMin || 1)) * ch;
    // Fundo + grid
...
function paginaLocalizacao(doc, avaliacao, corretor) {
    novaPagina(doc);
    microHeader(doc, corretor);
    tituloPagina(doc, "Localização");
    const endereco = String(avaliacao?.endereco_completo || "").trim() ||
...
function paginaAssinatura(doc, corretor, qrDataUrl) {
    novaPagina(doc);
    microHeader(doc, corretor);
    ...
    doc.setFontSize(10);
    doc.setTextColor(...TEXT);
    const decl = "Declaro que as informações contidas neste laudo são verdadeiras e foram obtidas através de pesquisa de mercado realizada na data de referência indicada, seguindo as orientações da NBR 14653-2 da ABNT.";
    ...
    const regs = [];
    if (corretor.creci)
        regs.push(`CRECI ${String(corretor.creci).replace(/^CRECI[\s:-]*/i, "")}`);
    if (corretor.cnai)
        regs.push(`CNAI ${String(corretor.cnai).replace(/^CNAI[\s:-]*/i, "")}`);
    if (corretor.outro_registro)
        regs.push(String(corretor.outro_registro));
    if (regs.length) {
        doc.setFont("helvetica", "normal");
        doc.setFontSize(9);
        doc.setTextColor(...GRAY);
    ...
    if (corretor.telefone)
        contato.push(corretor.telefone);
    if (corretor.email)
        contato.push(corretor.email);
    if (contato.length) {
        doc.setFontSize(9);
        doc.setTextColor(...GRAY);
...
function paginaMarketing(doc, marketing, corretor, avaliacao, resultado, fotoPrincipal) {
    ...
    y += 6;
    const canais = Array.isArray(marketing.divulgacao?.canais) ? marketing.divulgacao.canais : [];
    const prioridades = ["Alta", "Alta", "Média", "Média", "Baixa"];
    canais.slice(0, 5).forEach((c, i) => {
        const pri = prioridades[i] || "Média";
        const pct = pri === "Alta" ? 1 : pri === "Média" ? 0.6 : 0.3;
        const cor = pri === "Alta" ? GOLD : pri === "Média" ? BLUE : [120, 125, 135];
        doc.setFont("helvetica", "bold");
        doc.setFontSize(10);
        doc.setTextColor(...TEXT);
        doc.text(c, M, y + 4);
    ...
    const specsParts = [];
    if (qtd)
        specsParts.push(`${qtd} quarto${qtd > 1 ? "s" : ""}`);
    if (vgs)
        specsParts.push(`${vgs} vaga${vgs > 1 ? "s" : ""}`);
    if (ar)
        specsParts.push(`${fmtNum(ar, 0)} m²`);
    doc.setFont("helvetica", "normal");
    doc.setFontSize(9);
    doc.setTextColor(...GRAY);
...
function gerarModelo3(avaliacao, resultado, comparaveis, corretor, fotos, fotosDet = [], marketing, qrDataUrl) {
    const doc = new jsPDF({ unit: "mm", format: "a4", orientation: "portrait" });
    const rel = resultado?.relatorio_json || {};
    // EXPERT: até 10 fotos
...
export async function gerarPdfAvaliacao(avaliacao, resultado, comparaveis, opts) {
    ...
    const corretor = typeof opts.corretor === "string"
    ...
    if (modelo === 3) {
        try {
            const linhas = ["A8 AVALIA — Laudo de Avaliação"];
            if (avaliacao?.id)
                linhas.push(`Ref: ${String(avaliacao.id).slice(0, 8).toUpperCase()}`);
            if (corretor.nome)
                linhas.push(corretor.nome);
            if (corretor.telefone)
                linhas.push(`Tel: ${corretor.telefone}`);
            if (corretor.email)
                linhas.push(corretor.email);
