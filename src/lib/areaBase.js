 ...
 */
export function areaBaseDe(tipo, item) {
    ...
    if (tn.includes("casa") || tn.includes("sobrado")) {
        if (constr > 0)
            return { area: constr, fonte: "construida", label: "área construída" };
        if (priv > 0)
            return { area: priv, fonte: "privativa", label: "área construída" };
        return { area: total, fonte: "total", label: "área total" };
    }
    if (tn.includes("apart")) {
        if (priv > 0)
            return { area: priv, fonte: "privativa", label: "área privativa" };
        return { area: total, fonte: "total", label: "área total" };
    }
    // Galpão / barracão / sala comercial / loja / demais tipologias
    if (priv > 0)
        return { area: priv, fonte: "privativa", label: "área privativa/útil" };
    if (constr > 0)
        return { area: constr, fonte: "construida", label: "área construída" };
    return { area: total, fonte: "total", label: "área total" };
}
/** Rótulo do valor unitário exibido no PDF/tela, coerente com a área-base. */
export function labelValorM2(tipo) {
    const tn = String(tipo ?? "").toLowerCase();
    if (tn.includes("terreno") || tn.includes("lote"))
        return "Valor/m² terreno";
    if (tn.includes("casa") || tn.includes("sobrado"))
        return "Valor/m² construído";
    return "Valor/m² privativo";
}
/** Sufixo curto para eixos de gráfico e equações. */
export function sufixoAreaBase(tipo) {
    const tn = String(tipo ?? "").toLowerCase();
    if (tn.includes("terreno") || tn.includes("lote"))
        return "terreno";
    if (tn.includes("casa") || tn.includes("sobrado"))
