...
export const atualizarValorFinalCorretor = createServerFn({ method: "POST" })
    ...
    if (data.valor_final_corretor !== null) {
        ...
        if (Number.isFinite(min) && Number.isFinite(max)) {
            if (data.valor_final_corretor < min || data.valor_final_corretor > max) {
                throw new Error(`O valor deve estar entre ${min.toLocaleString("pt-BR", { style: "currency", currency: "BRL", maximumFractionDigits: 0 })} e ${max.toLocaleString("pt-BR", { style: "currency", currency: "BRL", maximumFractionDigits: 0 })} conforme o campo de arbítrio técnico (NBR 14653-2).`);
            }
        }
    }
    const { error: errU } = await supabase
        ...
        .eq("avaliacao_id", data.avaliacao_id);
    if (errU)
        throw new Error("Falha ao salvar valor: " + errU.message);
...
export function limiteEdicoesPorPlano(plano) {
    switch (plano) {
        case "expert":
...
export const getAvaliacaoDetalhe = createServerFn({ method: "GET" })
    ...
    if (errC)
        throw new Error("Erro ao carregar comparáveis");
    const authUser = userData?.user;
    const meta = (authUser?.user_metadata ?? {});
    const nomeCompleto = [meta.full_name, meta.name, meta.nome, profile?.nome].find((v) => typeof v === "string" && v.trim().length > 0) ??
...
export const regerarAvaliacao = createServerFn({ method: "POST" })
    ...
    const { error: errUpd } = await supabase
        ...
        .eq("id", data.id);
    if (errUpd)
        throw new Error("Falha ao atualizar avaliação: " + errUpd.message);
