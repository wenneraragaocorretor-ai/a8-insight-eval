...
import { gerarMarketingAvaliacao } from "../../lib/marketing.functions";
import { modelosDisponiveis } from "../../lib/pdfReport";
...
export const Route = createFileRoute("/_authenticated/avaliacoes/$id")({
    ...
    notFoundComponent: () => (<div className="p-8 text-center">
...
function AvaliacaoDetalhe() {
    ...
    const planoReal = (profile?.plano ?? "basico");
    ...
    const [modelo, setModelo] = useState(disponiveis[disponiveis.length - 1]);
    ...
    const camposExpertFaltando = (() => {
        ...
        const faltando = [];
        if (!a?.idade_real)
            faltando.push("Idade real do imóvel");
        if (!a?.posicao_solar)
            faltando.push("Posição solar");
        if (!a?.topografia)
            faltando.push("Topografia");
        if (!a?.zoneamento)
            faltando.push("Zoneamento");
    ...
    const valorFinalSalvo = resultado?.valor_final_corretor;
    const [valorFinalInput, setValorFinalInput] = useState(String(Number.isFinite(Number(valorFinalSalvo)) && Number(valorFinalSalvo) > 0
    ...
    const handleDownload = async () => {
        ...
        if (camposExpertFaltando.length > 0) {
            toast.error(`Esta avaliação foi criada antes do upgrade de plano e está faltando informações para o laudo completo (${camposExpertFaltando.join(", ")}). Edite a avaliação para preencher os campos da Ficha Técnica Detalhada antes de gerar este modelo de PDF.`, {
                duration: 12000,
                action: {
                    label: "Editar agora",
                    onClick: () => navigate({ to: "/avaliacoes/nova", search: { edit: avaliacao.id } }),
                },
            });
            return;
        }
        ...
        try {
            ...
            const fotosPaths = Array.isArray(avaliacao?.fotos) ? avaliacao.fotos : [];
            const fotosMeta = Array.isArray(avaliacao?.fotos_meta) ? avaliacao.fotos_meta : [];
            const fotosDataUrls = [];
            const fotosDetalhadas = [];
            ...
            const logoPath = profile?.logo_url;
            if (logoPath) {
                try {
            ...
            await gerarPdfAvaliacao(avaliacao, resultado, comparaveis, {
                ...
                corretor: {
                    ...
                    cnai: profile?.cnai ?? null,
                    outro_registro: profile?.outro_registro ?? null,
                    telefone: profile?.telefone ?? null,
                    cidade: profile?.cidade ?? null,
                    estado: profile?.estado ?? null,
                    email: profile?.email ?? null,
                    nome_imobiliaria: profile?.nome_imobiliaria ?? null,
    ...
    return (<div className="max-w-5xl mx-auto py-8 space-y-6">
      <div className="flex items-center justify-between flex-wrap gap-4">
        <div>
          ...
          <div className="flex items-center gap-3 mt-2 flex-wrap">
            ...
            {avaliacao?.editado && (<span className="text-xs font-semibold uppercase tracking-wide bg-brand-gold text-white px-2 py-1 rounded">
                Atualizado
              </span>)}
          </div>
          {(() => {
            ...
            const areaTxt = base.area > 0
                ? `${base.area} m² (${base.label})${Number.isFinite(total) && total > 0 && total !== base.area
                    ? ` | ${total} m² totais`
                    : ""}`
          ...
          {avaliacao?.ultima_edicao_em && (<p className="text-xs text-muted-foreground mt-1">
              Última edição em{" "}
              {new Date(avaliacao.ultima_edicao_em).toLocaleDateString("pt-BR", {
        ...
        <div className="flex flex-col items-end gap-2">
          <div className="flex items-end gap-2 flex-wrap">
            ...
            return (<SelectItem key={m.id} value={String(m.id)} disabled={bloqueado}>
            ...
            <Button variant="outline" onClick={() => navigate({ to: "/avaliacoes/nova", search: { edit: avaliacao.id } })} className="gap-2 border-[#0F2D5C] text-[#0F2D5C] hover:bg-[#0F2D5C] hover:text-white" disabled={(() => {
            if (avaliacao.id.startsWith("mock-"))
                return true; // Bloqueado no Mock
            const lim = limiteEdicoesPorPlano(plano);
            return lim !== null && (avaliacao?.edicoes_count ?? 0) >= lim;
        })()}>
              <Pencil size={16}/> {avaliacao.id.startsWith("mock-") ? "Edição bloqueada (Mock)" : "Editar Laudo"}
            </Button>

            <Button onClick={handleDownload} className="gap-2 bg-brand-gold text-primary-foreground" disabled={generatingPdf}>
              {generatingPdf ? (<>
          ...
          {(() => {
            const lim = limiteEdicoesPorPlano(plano);
            const usadas = avaliacao?.edicoes_count ?? 0;
            if (lim === null) {
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <Card className="premium-card border-brand-gold">
          ...
          <CardContent>
            ...
            const valorBase = Number.isFinite(vum) && vum > 0 ? vum : base.area > 0 ? vc / base.area : 0;
            const showTotalRef = base.fonte !== "total" &&
                Number.isFinite(total) &&
                total > 0 &&
                total !== base.area &&
                vc > 0;
            return (<>
                  {valorBase > 0 && (<p className="text-xs text-muted-foreground mt-1">{fmtBRL(valorBase)}/m² {baseLabel}</p>)}
                  {showTotalRef && (<p className="text-xs text-muted-foreground">{fmtBRL(vc / total)}/m² total (referência)</p>)}
                </>);
        })()}
          </CardContent>
        </Card>
            ...
            return (<div className="space-y-2">
                <div className="flex flex-wrap items-center gap-3">
                  <div className="flex items-center gap-2">
                    ...
                    <Input type="text" inputMode="numeric" value={valorFinalInput} onChange={(e) => setValorFinalInput(e.target.value)} className="w-44 text-lg font-semibold"/>
                  </div>
                  <Button onClick={handleSalvar} disabled={!valido || !alterado || savingValor} className="gap-2 bg-brand-gold text-primary-foreground">
                    <Save size={16}/> Salvar
                  </Button>
                  {valorFinalSalvo != null && (<Button variant="outline" onClick={handleResetar} disabled={savingValor} className="gap-2">
                      <RotateCcw size={16}/> Usar valor calculado
                    </Button>)}
                </div>
                {!valido ? (<p className="text-xs text-destructive">
                    ...
                    {valorFinalSalvo != null && Math.round(Number(valorFinalSalvo)) !== Math.round(valorCentralTecnico) && (<span> Valor personalizado salvo: <strong>{fmtBRL(Number(valorFinalSalvo))}</strong>.</span>)}
                  </p>)}
              </div>);
        })()}
        </CardContent>
      ...
      {rel.analise && (<Card className="premium-card">
      ...
      {(rel.dicas || rel.recomendacoes) && (<Card className="premium-card">
          <CardHeader><CardTitle>Dicas e Recomendações</CardTitle></CardHeader>
          <CardContent>
            {Array.isArray(rel.dicas || rel.recomendacoes) ? (<ul className="list-disc pl-5 space-y-2">
                {(rel.dicas || rel.recomendacoes).map((d, i) => (<li key={i} className="text-sm">{d}</li>))}
              </ul>) : (<p className="text-sm whitespace-pre-wrap">{String(rel.dicas || rel.recomendacoes)}</p>)}
          </CardContent>
        </Card>)}

      {/* ============ ASSISTENTE DE MARKETING (somente Expert) ============ */}
      {String(plano ?? "").toLowerCase() !== "expert" ? (<Card className="premium-card border-dashed border-brand-gold/50 bg-muted/30">
          ...
          <CardContent className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
            ...
            <Button onClick={() => navigate({ to: "/planos" })} className="bg-brand-gold text-primary-foreground gap-2 shrink-0">
              <Sparkles size={16}/> Fazer upgrade
            </Button>
          </CardContent>
        </Card>) : (<Card className="premium-card border-brand-gold">
        <CardHeader className="flex flex-row items-center justify-between gap-4">
          <CardTitle className="flex items-center gap-2">
            <Sparkles className="text-brand-gold" size={20}/>
            Assistente de Marketing
          </CardTitle>
          {!marketing && (<Button onClick={handleGerarMarketing} disabled={loadingMkt} className="bg-brand-gold text-primary-foreground gap-2">
              <Sparkles size={16}/>
              {loadingMkt ? "Gerando..." : "Gerar com IA"}
            </Button>)}
          {marketing && (<Button variant="outline" size="sm" onClick={handleGerarMarketing} disabled={loadingMkt}>
              {loadingMkt ? "Atualizando..." : "Regenerar"}
            </Button>)}
        </CardHeader>
        <CardContent>
          {!marketing && !loadingMkt && (<p className="text-sm text-muted-foreground">
          ...
          {marketing && (<div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
              {/* CARD 1 — Público */}
                ...
                <CardContent className="space-y-2 text-sm">
                  <div>
                    ...
                    <div className="flex flex-wrap gap-1 mt-1">
                      {marketing.divulgacao.canais.map((c, i) => (<span key={i} className="text-xs bg-brand-blue text-white px-2 py-0.5 rounded">{c}</span>))}
                    </div>
                  </div>
