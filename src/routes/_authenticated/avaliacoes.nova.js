...
const TIPOS_IMOVEL = ["Apartamento", "Casa", "Sobrado", "Terreno", "Sala Comercial", "Galpão"];
function NativeSelect({ value, options, onChange }) {
    return (<select value={value} onChange={(event) => safe(() => onChange(event.currentTarget.value))} className="flex h-9 w-full rounded-md border border-input bg-transparent px-3 py-2 text-sm shadow-sm outline-none ring-offset-background focus:ring-1 focus:ring-ring disabled:cursor-not-allowed disabled:opacity-50">
...
export const Route = createFileRoute("/_authenticated/avaliacoes/nova")({
    ...
    component: () => (<ErrorBoundary fallbackTitle="Erro no formulário de Nova Avaliação">
      <NovaAvaliacao />
    </ErrorBoundary>),
});
...
const calcularValorM2 = (valor, area) => {
...
function NovaAvaliacao() {
    ...
    const [planoReal, setPlanoReal] = useState("basico");
    ...
    const [statusUso, setStatusUso] = useState(null);
    ...
    const recarregarStatus = async () => {
        try {
            ...
            setStatusUso({
                ...
                creditosAvulsos: s.creditosAvulsos ?? 0,
                assinaturaAtiva: !!s.assinaturaAtiva,
            });
            if (s.isAdmin)
                setIsAdmin(true);
            if (s.plano)
                setPlanoReal(s.plano);
        }
        catch (e) {
    ...
    const expertAtingiuLimite = !isAdmin &&
        plano === "expert" &&
        statusUso !== null &&
    ...
    const comprarLaudoExtra = async () => {
        try {
            ...
            const origin = (window.top && window.top !== window.self ? window.top.location.origin : null) ??
    ...
    const [importUrlAberto, setImportUrlAberto] = useState({});
    const [importUrlValor, setImportUrlValor] = useState({});
    const [importandoIdx, setImportandoIdx] = useState(null);
    const importarComparavelPorUrl = async (index) => {
        ...
        try {
            ...
            const normConserv = (v) => {
                const s = String(v ?? "").toLowerCase();
                if (s.includes("ótim") || s.includes("otim") || s.includes("novo"))
                    return "Novo";
                if (s.includes("regular"))
                    return "Regular";
                if (s.includes("ruim") || s.includes("ruím"))
                    return "Ruim";
                return "Bom";
            };
            const num = (v) => {
            ...
            const novosDados = {
                ...
                caracteristicas: Array.isArray(d.caracteristicas)
                    ? d.caracteristicas.filter((c) => CARACTERISTICAS_COMPARAVEL.includes(String(c)))
                    : [],
            };
            updateComp(index, novosDados);
            // Feedback de campos não encontrados
            const faltantes = [];
            if (!novosDados.localizacao)
                faltantes.push("Localização");
            if (!novosDados.area || novosDados.area <= 0)
                faltantes.push("Área");
            if (!novosDados.valor || novosDados.valor <= 0)
        ...
        (async () => {
            try {
                ...
                if (Array.isArray(res.comparaveis) && res.comparaveis.length > 0) {
                    setComparaveis(res.comparaveis.map((c, i) => ({
                        id: i + 1,
                        fonte: c.fonte ?? "",
                ...
                const fotosPaths = Array.isArray(av.fotos) ? av.fotos : [];
                const fotosMeta = Array.isArray(av.fotos_meta) ? av.fotos_meta : [];
    ...
    const handleFotosSelected = async (filesList) => {
        if (!filesList)
            return;
        ...
        for (const file of aProcessar) {
            ...
            const tempItem = { path: "", previewUrl, uploading: true, legenda: "", principal: false };
            ...
            try {
                ...
                if (error)
                    throw error;
                setFotos((prev) => prev.map((f) => (f.previewUrl === previewUrl ? { ...f, path, uploading: false } : f)));
            }
            catch (err) {
    ...
    const removerFoto = async (idx) => {
        ...
        if (alvo.previewUrl?.startsWith("blob:"))
    ...
    const setImovelField = (key, value) => {
    ...
    const toggleCaracteristica = (opcao) => {
        safe(() => {
            setImovel((prev) => ({
                ...prev,
    ...
    const toggleCompCaracteristica = (index, opcao) => {
        safe(() => {
    ...
    const duplicarComparavel = (index) => {
        safe(() => {
    ...
    return (<div className="max-w-4xl mx-auto py-8">
      {errorDetail && (<div className="mb-6 p-6 bg-red-50 border-2 border-red-200 rounded-xl animate-in fade-in slide-in-from-top-4 duration-300">
          ...
          {errorDetail.stack && (<details className="mt-2">
          ...
          <Button variant="outline" size="sm" className="mt-4 border-red-200 text-red-700 hover:bg-red-100" onClick={() => setErrorDetail(null)}>
            Fechar aviso
          </Button>
        </div>)}
      ...
      <div className="mb-8 flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold text-brand-blue">
            ...
            {typeof window !== 'undefined' && localStorage.getItem('DEBUG_MODE') === 'true' && (<span className="ml-3 text-xs bg-blue-100 text-blue-700 px-2 py-1 rounded-full uppercase tracking-tighter">
                DIAGNÓSTICO
              </span>)}
          </h1>
          <p className="text-muted-foreground">
            ...
            ? `Atualize os dados e regenere o laudo com a IA.${(() => {
                ...
                if (lim === null)
                    return " Edições ilimitadas (Expert).";
        ...
        <div className="flex gap-2">
          {[1, 2, 3].map((s) => (<div key={s} className={`w-10 h-2 rounded-full ${step >= s ? "bg-brand-gold" : "bg-muted"}`}/>))}
        </div>
      </div>

      {step === 1 && (<Card className="premium-card">
          ...
          <CardContent className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="space-y-2">
              ...
              <Input placeholder="Ex: Itaim Bibi, São Paulo" value={imovel.localizacao} onChange={(e) => setImovelField("localizacao", e.target.value)}/>
            </div>
            <div className="space-y-2 md:col-span-2">
              <Label>Endereço completo do imóvel</Label>
              <Input placeholder="Ex: Rua Joaquim Floriano, 100, Itaim Bibi, São Paulo - SP, 04534-000" value={imovel.endereco_completo} onChange={(e) => setImovelField("endereco_completo", e.target.value)}/>
            ...
            <div className="space-y-2">
              ...
              <Input type="number" value={imovel.area_total} onChange={(e) => setImovelField("area_total", toNum(e.target.value))}/>
            </div>
            {campos.areaConstruida && (<div className="space-y-2">
                <Label>Área Construída (m²)</Label>
                <Input type="number" value={imovel.area_construida || ""} placeholder="Informe a área edificada" onChange={(e) => setImovelField("area_construida", toNum(e.target.value))}/>
              </div>)}
            {campos.areaPrivativa && (<div className="space-y-2">
                <Label>Área Privativa (m²)</Label>
                <Input type="number" value={imovel.area_privativa || ""} placeholder="Opcional" onChange={(e) => setImovelField("area_privativa", toNum(e.target.value))}/>
                {["Casa", "Apartamento", "Sobrado", "Galpão"].includes(imovel.tipo) &&
                    (!imovel.area_privativa || imovel.area_privativa <= 0) && (<p className="text-xs rounded-md border border-yellow-400 bg-yellow-50 text-yellow-800 px-3 py-2">
                      Informe a Área Privativa para um cálculo mais preciso — será usada como base do valor/m².
                    </p>)}
              </div>)}


            {(campos.quartos || campos.suites || campos.banheiros || campos.vagas) && (<div className="grid grid-cols-2 md:grid-cols-4 gap-4 md:col-span-2">
                {campos.quartos && (<div className="space-y-2">
                    <Label>Quartos</Label>
                    <Input type="number" value={imovel.quartos} onChange={(e) => setImovelField("quartos", toNum(e.target.value))}/>
                  </div>)}
                {campos.suites && (<div className="space-y-2">
                    <Label>Suítes</Label>
                    <Input type="number" value={imovel.suites} onChange={(e) => setImovelField("suites", toNum(e.target.value))}/>
                  </div>)}
                {campos.banheiros && (<div className="space-y-2">
                    <Label>Banheiros</Label>
                    <Input type="number" value={imovel.banheiros} onChange={(e) => setImovelField("banheiros", toNum(e.target.value))}/>
                  </div>)}
                {campos.vagas && (<div className="space-y-2">
                    <Label>Vagas</Label>
                    <Input type="number" value={imovel.vagas} onChange={(e) => setImovelField("vagas", toNum(e.target.value))}/>
                  </div>)}
              </div>)}

            {campos.andar && (<div className="space-y-2">
                <Label>Andar {imovel.tipo === "Apartamento" ? "*" : ""}</Label>
                <Input type="number" value={imovel.andar || ""} placeholder={imovel.tipo === "Apartamento" ? "Obrigatório" : "Opcional"} onChange={(e) => setImovelField("andar", toNum(e.target.value))}/>
              </div>)}
            ...
            {campos.conservacao && (<div className="space-y-2">
                <Label>Estado de Conservação</Label>
                <NativeSelect value={imovel.conservacao} options={CONSERVACOES} onChange={(v) => setImovelField("conservacao", v)}/>
              </div>)}

            {campos.caracteristicas && (<div className="space-y-3 md:col-span-2">
                <Label>Características Adicionais</Label>
                <div className="flex flex-wrap gap-4">
                  {CARACTERISTICAS_OPCOES.map((opcao) => (<label key={opcao} className="flex items-center gap-2 cursor-pointer">
                      <input type="checkbox" checked={imovel.caracteristicas.includes(opcao)} onChange={() => toggleCaracteristica(opcao)} className="h-4 w-4 shrink-0 cursor-pointer rounded-sm accent-brand-blue"/>
                      <span className="text-sm">{opcao}</span>
                    </label>))}
                </div>
              </div>)}

            {camposDoTipo(imovel.tipo).caracteristicas && (<div className="md:col-span-2 rounded-lg border border-border bg-muted/30 p-5 space-y-5">
                ...
                {[
                    {
                        ...
                        key: "ambientes_sociais",
                        outrosKey: "ambientes_sociais_outros",
                    },
                    {
                        titulo: "Ambientes de Serviço",
                        opcoes: AMBIENTES_SERVICO_OPCOES,
                        key: "ambientes_servico",
                        outrosKey: "ambientes_servico_outros",
                    },
                    {
                        titulo: "Outros Ambientes",
                        opcoes: AMBIENTES_OUTROS_OPCOES,
                        key: "ambientes_outros",
                        outrosKey: "ambientes_outros_livres",
                    },
                ].map((grupo) => (<div key={grupo.titulo} className="space-y-3">
                    <Label className="text-sm font-semibold uppercase tracking-wide text-brand-blue">
                      {grupo.titulo}
                    </Label>
                    <div className="flex flex-wrap gap-4">
                      {grupo.opcoes.map((opcao) => (<label key={opcao} className="flex items-center gap-2 cursor-pointer">
                          <input type="checkbox" checked={imovel[grupo.key].includes(opcao)} onChange={() => safe(() => setImovel((prev) => {
                    ...
                    <Input placeholder="Ex: Brinquedoteca, Sala de jogos..." value={imovel[grupo.outrosKey]} onChange={(e) => setImovelField(grupo.outrosKey, e.target.value)}/>
                    <p className="text-xs text-muted-foreground">
            ...
            {temFichaCompleta && (<div className="md:col-span-2 rounded-lg border border-brand-gold/40 bg-brand-gold/5 p-5 space-y-5">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="space-y-2">
                    ...
                    <Input type="number" placeholder="Opcional" value={imovel.idade_real || ""} onChange={(e) => setImovelField("idade_real", toNum(e.target.value))}/>
                  </div>
                  <div className="space-y-2">
                    <Label>Idade Aparente</Label>
                    <NativeSelect value={imovel.idade_aparente} options={["", ...IDADE_APARENTE_OPCOES]} onChange={(v) => setImovelField("idade_aparente", v)}/>
                  </div>
                  <div className="space-y-2">
                    <Label>Posição Solar</Label>
                    <NativeSelect value={imovel.posicao_solar} options={["", ...POSICAO_SOLAR_OPCOES]} onChange={(v) => setImovelField("posicao_solar", v)}/>
                  </div>
                  {(imovel.tipo === "Casa" || imovel.tipo === "Terreno") && (<div className="space-y-2">
                      <Label>Topografia</Label>
                      <NativeSelect value={imovel.topografia} options={["", ...TOPOGRAFIA_OPCOES]} onChange={(v) => setImovelField("topografia", v)}/>
                    </div>)}
                  <div className="space-y-2">
                    <Label>Zoneamento</Label>
                    <Input placeholder="Ex: ZR1, ZC, ZM" value={imovel.zoneamento} onChange={(e) => setImovelField("zoneamento", e.target.value)}/>
                  </div>
                  <div className="space-y-2">
                    <Label>Vagas Cobertas</Label>
                    <Input type="number" placeholder="Opcional" value={imovel.vagas_cobertas || ""} onChange={(e) => setImovelField("vagas_cobertas", toNum(e.target.value))}/>
                  </div>
                  <div className="space-y-2">
                    <Label>Vagas Descobertas</Label>
                    <Input type="number" placeholder="Opcional" value={imovel.vagas_descobertas || ""} onChange={(e) => setImovelField("vagas_descobertas", toNum(e.target.value))}/>
                  </div>
                  {imovel.tipo === "Apartamento" && (<div className="space-y-2">
                      <Label>Total de Andares do Edifício</Label>
                      <Input type="number" placeholder="Opcional" value={imovel.total_andares || ""} onChange={(e) => setImovelField("total_andares", toNum(e.target.value))}/>
                    </div>)}
                  {(imovel.tipo === "Casa" || imovel.tipo === "Sobrado") && (<div className="space-y-2">
                      <Label>Número de Pavimentos</Label>
                      <NativeSelect value={imovel.numero_pavimentos} options={["", ...NUMERO_PAVIMENTOS_OPCOES]} onChange={(v) => setImovelField("numero_pavimentos", v)}/>
                    </div>)}
                </div>
                ...
                <div className="space-y-3">
                  ...
                  <div className="flex flex-wrap gap-4">
                    {TIPO_ACABAMENTO_OPCOES.map((opcao) => (<label key={opcao} className="flex items-center gap-2 cursor-pointer">
                        <input type="checkbox" checked={imovel.tipo_acabamento.includes(opcao)} onChange={() => safe(() => setImovel((prev) => ({
                        ...prev,
                  ...
                  <div className="space-y-2 pt-2">
                    ...
                    <Input placeholder="Ex: Pedra natural, Deck de madeira..." value={imovel.acabamento_outros} onChange={(e) => setImovelField("acabamento_outros", e.target.value)}/>
                  </div>
                </div>
                ...
                <div className="space-y-3">
                  ...
                  <div className="flex flex-wrap gap-4">
                    {INFRA_LAZER_OPCOES.map((opcao) => (<label key={opcao} className="flex items-center gap-2 cursor-pointer">
                        <input type="checkbox" checked={imovel.infraestrutura_lazer.includes(opcao)} onChange={() => safe(() => setImovel((prev) => ({
                        ...prev,
                  ...
                  <div className="space-y-2 pt-2">
                    ...
                    <Input placeholder="Ex: Rooftop, Espaço zen, Pista de skate..." value={imovel.lazer_outros} onChange={(e) => setImovelField("lazer_outros", e.target.value)}/>
                  </div>
                </div>
              </div>)}
            ...
            <div className="space-y-2 md:col-span-2">
              ...
              <Input placeholder="Informações complementares..." value={imovel.observacoes} onChange={(e) => setImovelField("observacoes", e.target.value)}/>
            </div>
            ...
            <div className="space-y-3 md:col-span-2">
              <div className="flex items-end justify-between gap-4 flex-wrap">
                ...
                <Button type="button" variant="outline" className="gap-2" disabled={fotos.length >= maxFotos} onClick={() => fileInputRef.current?.click()}>
                  <ImagePlus size={16}/> Adicionar foto
                </Button>
                <input ref={fileInputRef} type="file" accept="image/jpeg,image/png,image/webp" multiple className="hidden" onChange={(e) => safe(() => handleFotosSelected(e.target.files))}/>
              </div>

              {fotos.length === 0 ? (<button type="button" onClick={() => fileInputRef.current?.click()} className="w-full border-2 border-dashed border-border rounded-lg py-8 flex flex-col items-center gap-2 text-muted-foreground hover:border-brand-gold hover:text-brand-gold transition-colors">
                  <Upload size={28}/>
                  <span className="text-sm">Clique para enviar fotos do imóvel</span>
                </button>) : (<div className="grid grid-cols-2 md:grid-cols-3 gap-3">
                  {fotos.map((f, i) => (<div key={f.previewUrl} className="space-y-2">
                      <div className="relative group rounded-lg overflow-hidden border border-border aspect-[4/3] bg-muted">
                        ...
                        {f.uploading && (<div className="absolute inset-0 bg-black/40 flex items-center justify-center text-white text-xs">
                            Enviando…
                          </div>)}
                        <button type="button" onClick={() => safe(() => setFotos((prev) => prev.map((p, idx) => ({ ...p, principal: idx === i ? !p.principal : false }))))} className={`absolute top-2 left-2 rounded-full p-1.5 ${f.principal ? "bg-brand-gold text-white" : "bg-black/70 text-white hover:bg-brand-gold"}`} aria-label={f.principal ? "Foto principal" : "Marcar como foto principal"} title={f.principal ? "Foto principal" : "Marcar como foto principal"}>
                          <Star size={14} fill={f.principal ? "currentColor" : "none"}/>
                        </button>
                        <button type="button" onClick={() => safe(() => removerFoto(i))} className="absolute top-2 right-2 bg-black/70 hover:bg-destructive text-white rounded-full p-1.5 opacity-90" aria-label="Remover foto">
                          <X size={14}/>
                        </button>
                      </div>
                      <div className="space-y-1">
                        <Label className="text-xs">Legenda da foto</Label>
                        <Input placeholder="Ex: Sala de estar, Cozinha, Fachada, Varanda..." value={f.legenda} onChange={(e) => safe(() => setFotos((prev) => prev.map((p, idx) => (idx === i ? { ...p, legenda: e.target.value } : p))))}/>
                      </div>
                    </div>))}
                </div>)}
            </div>
      ...
      {step === 2 && (<div className="space-y-6">
          ...
          {comparaveis.map((c, index) => (<Card key={c.id} className="premium-card">
              <CardHeader className="flex flex-row items-center justify-between pb-2">
                <CardTitle className="text-base">Comparável #{index + 1}</CardTitle>
                <div className="flex items-center gap-2">
                  <Button variant="outline" size="sm" type="button" onClick={() => setImportUrlAberto((s) => ({ ...s, [c.id]: !s[c.id] }))} className="gap-1">
                    📎 Importar por URL
                  </Button>
                  <Button variant="ghost" size="sm" type="button" onClick={() => duplicarComparavel(index)} className="gap-1 text-brand-blue">
              ...
              {importUrlAberto[c.id] && (<div className="px-6 pb-2 flex flex-col sm:flex-row gap-2">
                  <Input type="url" placeholder="Cole aqui o link do anúncio (Zap, OLX, Viva Real...)" value={importUrlValor[c.id] ?? ""} onChange={(e) => setImportUrlValor((s) => ({ ...s, [c.id]: e.target.value }))}/>
                  <Button type="button" onClick={() => importarComparavelPorUrl(index)} disabled={importandoIdx === index} className="bg-brand-blue">
              ...
              <CardContent className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div className="space-y-2">
                  ...
                  <Input value={c.fonte} onChange={(e) => updateComp(index, { fonte: extrairDominio(e.target.value) })} placeholder="https://www.zapimoveis.com.br/..."/>
                </div>
                <div className="space-y-2 md:col-span-2">
                  <Label className={validationErrors[`comp-${c.id}-localizacao`] ? "text-destructive" : ""}>
                    Localização / Referência
                  </Label>
                  <Input id={`comp-${c.id}-localizacao`} name={`comp-${c.id}-localizacao`} value={c.localizacao} onChange={(e) => {
                  ...
                  {validationErrors[`comp-${c.id}-localizacao`] && (<p className="text-xs font-medium text-destructive">
                ...
                <div className="space-y-2">
                  ...
                  <Input id={`comp-${c.id}-area`} name={`comp-${c.id}-area`} type="number" value={c.area || ""} onChange={(e) => {
                  ...
                  {validationErrors[`comp-${c.id}-area`] ? (<p className="text-xs font-medium text-destructive">
                      Comparável #{index + 1}: {validationErrors[`comp-${c.id}-area`]}
                    </p>) : (Number(c.valor) > 0 && (!c.area || Number(c.area) <= 0)) && (<p className="text-xs font-medium text-amber-600">
                      Informe a área para calcular o valor/m²
                    </p>)}
                </div>
                {campos.areaConstruida && (<div className="space-y-2">
                    <Label>Área Construída (m²)</Label>
                    <Input type="number" value={c.area_construida || ""} placeholder="Opcional" onChange={(e) => updateComp(index, { area_construida: toNum(e.target.value) })}/>
                  </div>)}
                {campos.areaPrivativa && (<div className="space-y-2">
                    <Label>Área Privativa (m²)</Label>
                    <Input type="number" value={c.area_privativa || ""} placeholder="Opcional" onChange={(e) => updateComp(index, { area_privativa: toNum(e.target.value) })}/>
                  </div>)}
                <div className="space-y-2">
                  <Label className={validationErrors[`comp-${c.id}-valor`] ? "text-destructive" : ""}>
                    Valor Anunciado (R$)
                  </Label>
                  <Input id={`comp-${c.id}-valor`} name={`comp-${c.id}-valor`} type="number" value={c.valor || ""} onChange={(e) => {
                  ...
                  {validationErrors[`comp-${c.id}-valor`] ? (<p className="text-xs font-medium text-destructive">
                      Comparável #{index + 1}: {validationErrors[`comp-${c.id}-valor`]}
                    </p>) : Number(c.valor) > 0 && Number(c.valor) < 10000 && (<p className="text-xs font-medium text-destructive">
                  ...
                  {(() => {
                    ...
                    if (vpm2 < 500 || vpm2 > 50000) {
                        return (<p className="text-xs font-medium text-amber-600">
                ...
                {campos.quartos && (<div className="space-y-2">
                    <Label>Quartos</Label>
                    <Input type="number" value={c.quartos} onChange={(e) => updateComp(index, { quartos: toNum(e.target.value) })}/>
                  </div>)}
                {campos.suites && (<div className="space-y-2">
                    <Label>Suítes</Label>
                    <Input type="number" value={c.suites} onChange={(e) => updateComp(index, { suites: toNum(e.target.value) })}/>
                  </div>)}
                {campos.banheiros && (<div className="space-y-2">
                    <Label>Banheiros</Label>
                    <Input type="number" value={c.banheiros} onChange={(e) => updateComp(index, { banheiros: toNum(e.target.value) })}/>
                  </div>)}
                {campos.vagas && (<div className="space-y-2">
                    <Label>Vagas</Label>
                    <Input type="number" value={c.vagas} onChange={(e) => updateComp(index, { vagas: toNum(e.target.value) })}/>
                  </div>)}
                <div className="space-y-2">
                  <Label>Padrão Construtivo</Label>
                ...
                {campos.andar && (<div className="space-y-2">
                    <Label>Andar</Label>
                    <Input type="number" value={c.andar || ""} placeholder="Opcional" onChange={(e) => updateComp(index, { andar: toNum(e.target.value) })}/>
                  </div>)}
                <div className="space-y-2">
                  <Label>Idade aprox. (anos)</Label>
                  <Input type="number" value={c.idade || ""} placeholder="Opcional" onChange={(e) => updateComp(index, { idade: toNum(e.target.value) })}/>
                </div>
                <div className="space-y-2">
                  <Label>Condomínio mensal (R$)</Label>
                  <Input type="number" value={c.condominio || ""} placeholder="Opcional" onChange={(e) => updateComp(index, { condominio: toNum(e.target.value) })}/>
                </div>
                {campos.caracteristicas && (<div className="space-y-3 md:col-span-3">
                    <Label>Características presentes</Label>
                    <div className="flex flex-wrap gap-4">
                      {CARACTERISTICAS_COMPARAVEL.map((opcao) => (<label key={opcao} className="flex items-center gap-2 cursor-pointer">
                          <input type="checkbox" checked={c.caracteristicas.includes(opcao)} onChange={() => toggleCompCaracteristica(index, opcao)} className="h-4 w-4 shrink-0 cursor-pointer rounded-sm accent-brand-blue"/>
                          <span className="text-sm">{opcao}</span>
                        </label>))}
                    </div>
                  </div>)}
      ...
      {step === 3 && (<Card className="premium-card text-center py-12">
          <CardContent className="space-y-6">
            ...
            {statusUso?.assinaturaAtiva === false && statusUso?.creditosAvulsos <= 0 && (<div className="bg-red-50 border border-red-200 rounded-lg p-4 mb-4 text-left">
                ...
                <Button variant="link" className="p-0 h-auto text-xs text-brand-blue underline mt-2" onClick={() => navigate({ to: "/planos" })}>
                  Ver planos e preços
                </Button>
              </div>)}

            <div className="flex flex-col gap-4 max-w-sm mx-auto w-full">
              <Button onClick={(e) => { e.preventDefault(); e.stopPropagation(); handleProcessar(); }} className={isEdit ? "bg-[#0F2D5C] text-white h-12 text-lg font-bold gap-2 hover:bg-[#0A1F44]" : "bg-brand-gold text-primary-foreground h-12 text-lg font-bold gap-2"} disabled={isLoading || (statusUso?.assinaturaAtiva === false && statusUso?.creditosAvulsos <= 0 && !isEdit)}>
              ...
              {!isLoading && (<Button variant="ghost" onClick={() => setStep(2)}>Revisar dados</Button>)}
            </div>
          </CardContent>
        </Card>)}
      ...
      {typeof window !== 'undefined' && localStorage.getItem('DEBUG_MODE') === 'true' && (<div className="fixed bottom-4 right-4 z-[9999]">
      ...
      <Dialog open={showLimiteModal} onOpenChange={setShowLimiteModal}>
        <DialogContent className="sm:max-w-md">
          ...
          <DialogFooter className="flex flex-col sm:flex-row gap-2">
            <Button variant="outline" className="w-full sm:w-auto" onClick={() => setShowLimiteModal(false)} disabled={comprandoExtra}>
              Aguardar renovação
            </Button>
            <Button className="w-full sm:w-auto bg-brand-gold text-primary-foreground hover:opacity-90" onClick={comprarLaudoExtra} disabled={comprandoExtra}>
