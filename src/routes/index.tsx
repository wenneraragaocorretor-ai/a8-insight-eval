/* O plano está aprovado com ajustes obrigatórios antes da implementação:

1. Defina um único mecanismo de pesquisa. Priorize a ferramenta oficial Web Search da API da Anthropic, utilizando a chave já armazenada no backend.

2. Não crie WEB_SEARCH_API_KEY nem contrate Tavily ou Serper sem antes confirmar tecnicamente que a busca oficial da Anthropic não atende ao fluxo.

3. Não prometa garantir cinco comparáveis. A função deve tentar atingir pelo menos 5 válidos, mas retornar “amostra insuficiente” quando isso não for possível, sem inventar dados.

4. Comparáveis fornecidos pelo usuário devem ser preservados, validados e identificados separadamente dos encontrados automaticamente.

5. A pesquisa automática deve buscar apenas a quantidade necessária para completar a amostra, evitando custos desnecessários.

6. Implemente limites:

- máximo de 5 pesquisas por avaliação;

- máximo de 15 URLs processadas;

- timeout por página;

- tratamento de sites bloqueados ou inacessíveis;

- tratamento dos erros 401, 402 e 429;

- proteção contra chamadas repetidas;

- confirmação do usuário antes de gerar custo.

7. Não considere snippets de resultados como comparáveis completos. Um comparável só será válido quando possuir, no mínimo:

- URL direta do anúncio;
...
8. Salve a origem de cada comparável como:

- fornecido_pelo_usuario; ou

- busca_automatica.

9. Faça deduplicação por URL normalizada e também pela combinação aproximada de localização, área, preço, quartos e características.

10. Não considere o preço anunciado como preço efetivo de transação. Identifique-o expressamente como “preço de oferta”.

11. Antes do cálculo final, apresente os comparáveis para aprovação, exclusão ou inclusão pelo usuário.

12. Não altere nem quebre as funções gerar-avaliacao e extrair-comparavel que já estão funcionando.

13. Atualize também /admin/test-ia para permitir:

- teste de extração por URL;

- teste de extração por texto;

- teste da busca automática;

- visualização dos descartes e respectivos motivos.

Implemente por etapas. Primeiro crie a função e os testes; depois integre-a ao fluxo da nova avaliação. Ao concluir a primeira etapa, informe os arquivos alterados e apresente os resultados dos testes antes de continuar. */
